...
// A custom LLM wrapper for OpenRouter using Axios
class OpenRouterLLM extends llm.LLM {
    ...
    async chat(ctx, chatCtx, fallback) {
        ...
        try {
            ...
            // Extract Action Tags to send via Data Channel
            ...
            if (actionMatch && ctx.room) {
               ...
               // Send Data Channel message to Android app
...
const agent = defineAgent({
    entry: async (ctx) => {
        ...
        // Using basic Google GenAI TTS (ensure GOOGLE_API_KEY is in env)
        const { tts } = require("@google/genai"); 

        const agentPipeline = new pipeline.VoicePipelineAgent(
            ctx,
            openRouter,
            null, // STT (Using client-side STT or LiveKit deepgram)
            null  // TTS 
        );
